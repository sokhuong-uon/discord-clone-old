<template>
<div @mouseover="hoverChat = true" @mouseleave="hoverChat = false" class="relative w-full h-auto mt-4 min-w-0 pr-12 flex hover:bg-nightgray">
	<div class="w-12 h-12 mx-3 flex-shrink-0 flex items-center justify-center">
	...
	<div class="w-full h-auto min-w-0 inline-block flex-shrink">
		...
		<div v-if="editing" class="text-sm text-gray-600">
			escape to
			<a @click="editing = false" role="button" class="text-indigo-600">cancel</a>
			• enter to
			<a @click="editing = false" role="button" class="text-indigo-600">save</a>
		</div>
	</div>
	<div v-if="hoverChat" class="absolute top-0 right-0">
		<div class="bg-nightgray text-gray-600 flex items-center rounded-md">
			<button class="btn w-8 h-8 hover:bg-nightgraylighter flex items-center justify-center">
				<svg class="icon-3Gkjwa" aria-hidden="false" width="24" height="24"><path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M12.251 2.003A10.232 10.232 0 0012 2C6.477 2 2 6.477 2 12c0 5.522 4.477 10 10 10s10-4.478 10-10a9.99 9.99 0 00-.003-.251 8 8 0 01-9.746-9.746zM10 8a2 2 0 10-4.001.001A2 2 0 0010 8zm2 11c3.14 0 6-2.383 6-5v-1H6v1c0 2.617 2.86 5 6 5z"/><path d="M21 3V0h-2v3h-3v2h3v3h2V5h3V3h-3z" fill="currentColor"/></svg>
			</button>
			<button class="btn w-8 h-8 hover:bg-nightgraylighter flex items-center justify-center" @click="editing = true">
			...
			<button class="btn w-8 h-8 hover:bg-nightgraylighter flex items-center justify-center">
				<svg class="icon-3Gkjwa" aria-hidden="false" width="24" height="24"><path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M7 12.001a2 2 0 10-4 0 2 2 0 004 0zm7 0a2 2 0 10-4 0 2 2 0 004 0zm5-2a2 2 0 110 4 2 2 0 010-4z"/></svg>
			</button>
		</div>
	</div>
</div>
