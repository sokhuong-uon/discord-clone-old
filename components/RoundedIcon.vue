<template>
	<div class="relative w-full flex justify-center h-12 mb-2">
		<div class="absolute flex items-center w-3 h-12 left-0">
			<span :class="{'h-8':isHover}" class="absolute w-4/12 h-2 bg-green-700 rounded-r-lg"></span>
		</div>
		<div @mouseover="isHover = true" @mouseleave="isHover = false" class="w-12 h-12 flex justify-start">
			<nuxt-link v-if="type=='server'" :class="{'':isHover}" class="" to="#">
				...
				/>
				<div v-else class="w-12 h-12 flex justify-center items-center bg-gray-800 rounded-full overflow-hidden text-gray-200">
			...
			<button v-if="type=='button'" class="w-12 h-12">
				<div :class="{'bg-green-600':isHover}" class="w-12 h-12 bg-gray-800 flex justify-center items-center rounded-full overflow-hidden">
					<svg :class="{'w-12 h-6 mt-1 mr-1 transform mx-auto':button.name=='Home'}" class="w-6 h-6 text-green-600" viewBox="0 0 24 24">
						<path
